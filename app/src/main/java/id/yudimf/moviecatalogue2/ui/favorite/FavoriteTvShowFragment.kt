...
class FavoriteTvShowFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        movieViewModel.allTvShow.observe(viewLifecycleOwner, Observer {
            it?.let {
                list.clear()
                list.addAll(it)
                showLoading(false)
        ...
        rv_favorite_tv_show.layoutManager = LinearLayoutManager(context)
        rv_favorite_tv_show.adapter = cardViewAdapter
    ...
    private fun showLoading(state: Boolean) {
        if (state) {
            progressBarFavoriteTvShow.visibility = View.VISIBLE
        } else {
            progressBarFavoriteTvShow.visibility = View.GONE
        }
    }
